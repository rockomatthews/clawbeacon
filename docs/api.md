...
This guide covers the complete REST API for Claw Beacon. All endpoints return JSON.
...
Currently, Claw Beacon does not implement rate limiting. For production deployments, consider adding a reverse proxy (nginx, Cloudflare) with rate limiting.
