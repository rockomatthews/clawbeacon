...
Claw Beacon includes a built-in **Model Context Protocol (MCP)** server, allowing AI agents like Claude to interact with your mission control directly.
...
Claw Beacon's MCP server exposes task and agent management as MCP tools, so your AI can:
...
1. Claw Beacon backend running (see [Getting Started](./getting-started.md))
...
Add Claw Beacon to your MCP configuration file.
...
```json
{
  "mcpServers": {
    "claw-beacon": {
      "command": "node",
      "args": ["/path/to/claw-beacon/packages/backend/src/mcp-server.js"],
      "env": {
        "DATABASE_URL": "sqlite:/path/to/claw-beacon/packages/backend/data/claw-beacon.db"
      }
    }
  }
}
...
```json
{
  "mcpServers": {
    "claw-beacon": {
      "command": "node",
      "args": ["C:\\path\\to\\claw-beacon\\packages\\backend\\src\\mcp-server.js"],
      "env": {
        "DATABASE_URL": "sqlite:C:\\path\\to\\claw-beacon\\packages\\backend\\data\\claw-beacon.db"
      }
    }
  }
}
...
You should see Claw Beacon tools like `list_tasks`, `create_task`, etc.
...
```json
{
  "mcpServers": {
    "claw-beacon": {
      "command": "node",
      "args": ["/opt/claw-beacon/packages/backend/src/mcp-server.js"],
      "env": {
...
Here are practical examples of how to use Claw Beacon with Claude:
...
```bash
cd packages/backend
DATABASE_URL=sqlite:./data/claw-beacon.db node src/mcp-server.js
```

If it starts successfully, you'll see:
```
Claw Beacon MCP server running on stdio
```
