...
This guide covers all configuration options for Claw Beacon, including agent definitions, webhooks, and environment variables.
...
Webhooks allow Claw Beacon to notify external services when events occur.
...
If you provide a `secret`, Claw Beacon adds an HMAC-SHA256 signature header:
...
# Absolute path
DATABASE_URL=sqlite:/var/lib/claw-beacon/database.db
```
...
# With SSL
DATABASE_URL=postgresql://user:password@host:5432/claw_beacon?sslmode=require

# Railway/Heroku style
DATABASE_URL=postgres://user:password@host:5432/claw_beacon
```
...
```env
# packages/backend/.env
DATABASE_URL=postgresql://user:password@db-host:5432/claw_beacon
PORT=3001
