...
This guide explains how to connect your OpenClaw agents to Claw Beacon for real-time task management and coordination.
...
```javascript
#!/usr/bin/env node
const API_URL = process.env.claw_beacon_URL || 'http://localhost:3001';
...
When you are spawned as a sub-agent to perform a task, update the Claw Beacon Dashboard.
...
```
claw_beacon_URL=https://your-claw-beacon-backend.railway.app
```
...
### Update Agent Status
```bash
curl -X PUT $claw_beacon_URL/api/agents/1 \
...
```bash
curl -X POST $claw_beacon_URL/api/messages \
  -H "Content-Type: application/json" \
...
### Update Task Status
```bash
curl -X PUT $claw_beacon_URL/api/tasks/123 \
  -H "Content-Type: application/json" \
  -d '{"status": "completed"}'
```
...
Claw Beacon includes an MCP server for direct Claude Desktop integration:
