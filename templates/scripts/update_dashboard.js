...
const claw_beacon_URL = process.env.claw_beacon_URL || 'http://localhost:3001';
...
async function main() {
  ...
  try {
    ...
    if (status) {
      ...
      const res = await fetch(`${claw_beacon_URL}/api/agents/${agentId}`, {
        method: 'PUT',
        headers,
    ...
    if (message) {
      const res = await fetch(`${claw_beacon_URL}/api/messages`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ 
          agent_id: agentId, 
    ...
    console.error('Error connecting to Claw Beacon:', err.message);
    console.error(`Make sure claw_beacon_URL is set correctly: ${claw_beacon_URL}`);
    process.exit(1);
  }
}
